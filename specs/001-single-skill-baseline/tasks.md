...
Project history). Each needs a decision before T057/T051/T025 can close.

- [ ] T058 R001-002 (HIGH) — the immutable HEAD cannot pass its own `implement_evidence` prerequisite (T025/T051/T057 unchecked ⇒ `TASKS_INCOMPLETE`). **Decision needed:** (a) drive the flow to an `APPROVED` re-run and commit truthful completion, or (b) revise the durable lifecycle contract so acceptance/homologation tasks are not gated by the prerequisite (remove the circular gate). Not resolvable by simply ticking the boxes.
- [ ] T059 R001-001 (HIGH) — a normal `update` leaves retired runtimes (`powerpack_debt.py`, `powerpack_full_cycle.py`), their configs, and stale routing/prereq keys on an already-installed project; `install_support` stopped copying them but never unlinks, and `write_json` keeps old config unless `--reset-config`. **Scope decision needed:** fix with a versioned prune step (touches FR-003/FR-012), or confirm out-of-scope per spec.md "no migration shim is provided" (FR-006/FR-010 target fresh install state).
- [ ] T060 R001-003 (MED) — `test_baseline_contract.py::test_installed_support_has_no_removed_command_assets` never asserts `installed_powerpack_commands == {speckit.implement-review}` through the real install/preset boundary (FR-011 installed-side equality). Add that coverage (or point to an existing test that already does it).
- [ ] T061 R001-004 (MED) — `test_removed_command_names_are_unknown_to_the_runtime_parser` only checks argparse choices + `hasattr`; FR-014 requires invoking a removed command name at the registration/dispatch layer against a clean install. Strengthen the check.
- [ ] T062 After T058–T061: re-run `T025-validation-runbook.md` S6 against the new HEAD; require `APPROVED` (or a documented contract-level resolution for T058) + `review_protocol.py validate` clean, then close T025/T051/T057.
