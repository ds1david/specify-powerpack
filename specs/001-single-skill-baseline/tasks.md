...
- [X] T025 [US2] [ACCEPTANCE] Execute quickstart §6: readiness → convergence → quality gate → Sol route → browserless review; **no step fails due to a removed command/helper (SC-003)**. **DONE 2026-09-10.** Live round-trip executed against `349a6f6` (PR #15 head): runbook S1–S7 all valid, S6 reached a **valid terminal verdict** (`CHANGES_REQUIRED`), browserless confirmed, real `github.*` Codex-App calls, **zero removed-command failures** → SC-003 process property proven at runtime. Evidence: `T025-evidence/` (`RESULT.md`, `review.json`, S1–S7, offline collector 11/11 PASS). The `CHANGES_REQUIRED` findings were fixed in Phase 11 (T058–T063, CI green). A re-run on the post-Phase-11 HEAD hit the Codex usage limit → `BLOCKED (environmental)` per `T025-validation-runbook.md §5` (review budget); not a T025 failure. `APPROVED` is not a T025 criterion.
...
- [X] T051 [ACCEPTANCE] Live-service portion of T025 — **DONE 2026-09-10** with T025/T057: the round-trip executed and reached a valid terminal verdict; the live services (Codex + ChatGPT Project + GitHub App) all responded. Evidence in `T025-evidence/`.
...
- [X] T057 [ACCEPTANCE] T025/T051 live-service round-trip — **DONE.** Executed 2026-09-10 against `349a6f6` (PR #15 head): S1–S7 valid, S6 browserless (`browser/cdp/playwright/web2api = false`) via real `github.*` Codex-App calls, valid terminal verdict, zero removed-command failure. `CHANGES_REQUIRED` findings → Phase 11 (fixed, CI green). Post-Phase-11 re-run BLOCKED on Codex usage limit (`§5` environmental, not a failure). Evidence: `T025-evidence/`.
...
- [X] T062 [ACCEPTANCE] Re-run S6 on the post-Phase-11 HEAD — **attempted 2026-09-10, `BLOCKED (environmental)` on the Codex usage limit** (`T025-validation-runbook.md §5`: "review budget exhausted" is not a T025 failure). The offline collector re-ran **11/11 PASS** against the new HEAD (`b07c660`), and Phase 11's fixes are CI-green, so the round-3 changes are validated by the offline layer + CI; a fresh live verdict awaits available Codex budget and is not blocking. Round-1 `review.json` (`CHANGES_REQUIRED`, head `349a6f6`) stands as the completed-round-trip evidence.
