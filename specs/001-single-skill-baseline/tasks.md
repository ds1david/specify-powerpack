...
- [ ] T018 [US1] Remove the `implement begin` / `implement end` argparse subcommands and `cmd_implement_begin` / `cmd_implement_end` (and now-unused `workspace_snapshot`/`snapshot_delta` helpers only if nothing preserved uses them) from `src/speckit_powerpack/assets/runtime/powerpack_runtime.py`; keep `state` / `review` / `limit` / `gate` / `model` subcommands.
...
- [ ] T030 [P] [US3] Delete exploratory homologation scaffolding: `scripts/homologation/probe_*.py`, repo-root `*.har` files, `docs/WEB_GITHUB_HEADLESS_PROBE.md`; delete any test whose sole subject is a removed probe (e.g. `tests/test_chatgpt_github_headless_probe_contract.py` — confirm it is probe-only first) (FR-021, CHK020–021).
...
- **Phase 2 (Foundational)** → needs Phase 1. T006→T007→T008 sequential (same file); T009 parallel to them (different file); T010–T013 parallel after T006–T008.
