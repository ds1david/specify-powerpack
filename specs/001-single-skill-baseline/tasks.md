...
- [X] T006 Add an evidence-based prerequisite evaluator to `src/speckit_powerpack/assets/runtime/powerpack_runtime.py`: `implement_evidence(root, feature) -> dict` implementing the algorithm in `contracts/implement-review-prereq.md` (committed tasks.md + all implementation checkboxes `[X]` + non-documentation delta via Git; fail closed with `GIT_UNAVAILABLE` when a committed snapshot cannot be proven).
...
- [X] T010 [P] Write `tests/test_powerpack_runtime.py` cases for `implement_evidence`: `OK`, `MISSING_TASKS`, `TASKS_INCOMPLETE` (with `unchecked` count), `NO_IMPLEMENTATION_DELTA` (docs-only change), `GIT_UNAVAILABLE` fail-closed path, and contaminated SPEC-anchor rejection. Remove the old `implement begin/end` receipt-gate cases.
...
- [ ] T025 [US2] [ACCEPTANCE] Execute quickstart §6 on a fixture project: readiness (`specify-powerpack doctor`, `review status`) → convergence via upstream `speckit-converge` → quality gate → Sol route → browserless review; record that no step fails due to a removed command/helper (SC-003). **Blocked on live env**: needs Codex login, a repo↔ChatGPT-Project binding and a real GitHub PR. Static coverage in place: `test_powerpack_runtime` (evidence gate, gate detection), `test_browserless_review`, `test_review_*`. **Execution runbook:** `T025-validation-runbook.md`. **First round-trip executed 2026-09-10 → `CHANGES_REQUIRED`; see Phase 11.**
...
- [ ] T051 [ACCEPTANCE] T025 acceptance test (full browserless `implement-review` flow) — the **live-service** portion only. Runbook: `T025-validation-runbook.md`. First round-trip executed 2026-09-10 (`T025-evidence/`); superseded by T062 (re-run after the round-3 fixes). Closes with T057/T062.
...
## Phase 10: PR #15 review remediation — round 2

Second review of PR #15 against the SPEC-001 contract (2026-09-10). Two HIGH functional
findings; the SPEC, contract and code had diverged from each other on FR-018a semantics.

- [X] T053 HIGH: checkbox state read from the working tree contradicted FR-018a ("the working tree MUST NOT be consulted"). `implement_evidence` now reads `git show HEAD:<feature>/tasks.md` (new `git_show` helper, UTF-8 decode for Windows CI; `count_task_checkboxes` split out of `task_checkbox_counts`); working-tree read is kept only for `tasks.md` *existence*. Regression test `test_implement_evidence_reads_task_checkboxes_from_head_not_working_tree` (committed `[ ]` + working-tree `[X]` + committed code delta ⇒ `TASKS_INCOMPLETE`).
- [X] T054 HIGH: `feature_base_commit` returned the introduction commit's **parent**, so a non-doc change bundled into the commit that introduced `plan.md`/`tasks.md` counted as implementation evidence with no later commit — contradicting spec.md FR-018a ("the diff from the commit that introduced the SPEC's plan/tasks to HEAD"). Both runtimes now anchor on the introduction commit itself (delta = `git diff <anchor>..HEAD`, strictly after it); parent lookup removed. Regression test `test_implement_evidence_rejects_code_bundled_into_spec_introduction_commit`.
- [X] T055 Every FR-018a surface converged on one definition (checkbox state + delta from `HEAD`; anchor = SPEC introduction commit; delta strictly after it): `spec.md` FR-018a + new `## PR #15 review — round 2` clarification; `research.md` (steps 1–2 + third "Revised again" note); `data-model.md` (State row + new checkbox-state row); `contracts/implement-review-prereq.md` (inputs table + Evaluation steps 1–6 rewritten in actual execution order + "Why SPEC-scoped" + Non-goals); `speckit.implement-review.md` "Mandatory predecessor"; `quickstart.md §5` negative checks; `T025-validation-runbook.md` P6 + `S2-negatives.txt` row; runtimes `powerpack_runtime.py` + `powerpack_capabilities.py`.
- [X] T056 Full suite green locally (`pytest` = 100). CI matrix re-run required on the pushed commit.
- [ ] T057 [ACCEPTANCE] T025/T051 live-service round-trip — **EXECUTED 2026-09-10** against `349a6f6` (= PR #15 head) from a self-installed worktree. Runbook S1–S7 all reachable/valid; S6 `review run --path . --pr 15` completed **browserless** (`browser_used/cdp_used/playwright_used/web2api_used = false`) through real `github.*` Codex-App tool calls and reached a **valid terminal verdict**. **No step failed due to a removed command / helper / config** → SC-003's process property holds at runtime. Evidence: `T025-evidence/` (`RESULT.md`, `review.json`, `S1`–`S7`, offline collector 11/11 PASS). **Stays open** because the deep-review verdict is `CHANGES_REQUIRED`, not `APPROVED` — see Phase 11. First S6 attempt hit the 600 s default timeout; re-ran with `--timeout 3300`. Project serialized context failed to load ("conversation could not be loaded") — GitHub side fully worked.

## Phase 11: T057 deep-review findings — round 3 remediation

The live browserless deep review (`T025-evidence/review.json`, reviewer `codex`, round 1)
returned `CHANGES_REQUIRED` with 4 findings. It ran on SPEC + GitHub evidence only (no
Project history). User decisions (2026-09-10): T058 → revise the lifecycle contract;
T059 → fix with a versioned prune.

- [X] T058 R001-002 (HIGH) — **decouple `[ACCEPTANCE]` tasks from the FR-018a gate.** `count_implementation_checkboxes()` in `powerpack_runtime.py` skips any checkbox line tagged `[ACCEPTANCE]`; `implement_evidence` uses it. Homologation evidence becomes a PR-review artefact (`T025-evidence/`), not a runtime signal. Surfaces converged: `spec.md` FR-018a + round-3 clarification; `contracts/implement-review-prereq.md`; `speckit.implement-review.md`; `research.md`; `data-model.md`; `quickstart.md §5`; `T025-validation-runbook.md` P6. Tests: `test_implement_evidence_ignores_acceptance_tasks`, `test_implement_evidence_still_blocks_on_unchecked_implementation_task`. T025/T051/T057 (+ this phase's own acceptance items) tagged `[ACCEPTANCE]`.
