...
- [ ] T051 T025 acceptance test (full browserless `implement-review` flow) — still pending a live Codex + ChatGPT Project + GitHub PR. Runbook: `T025-validation-runbook.md`. Duplicates the intent of T025; kept as the single open item.
- [ ] T052 Windows `install.ps1` acceptance — covered by the CI matrix (`ubuntu/windows/macos × py 3.11/3.13`), which runs once PR #15 is marked ready-for-review (draft PRs skip CI by project policy). No code change; flip when the CI run on a non-draft revision is green.
