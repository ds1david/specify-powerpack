...
- [X] T053 HIGH: checkbox state read from the working tree contradicted FR-018a ("the working tree MUST NOT be consulted"). `implement_evidence` now reads `git show HEAD:<feature>/tasks.md` (new `git_show` helper, UTF-8 decode for Windows CI; `count_task_checkboxes` split out of `task_checkbox_counts`); working-tree read kept only for `tasks.md` *existence* and the git-unavailable degraded scan. Regression test `test_implement_evidence_reads_task_checkboxes_from_head_not_working_tree` (committed `[ ]` + working-tree `[X]` + committed code delta ⇒ `TASKS_INCOMPLETE`).
...
- [ ] T063 `.devcontainer/` homologation env: `devcontainer.json` (bind-mounts `~/.codex` / `~/.claude` / `~/.config/gh`, symlinks the standalone `codex`), `homologate.sh <PR>` (worktree → install → bind → runbook S1/S6/S7 + offline collector → cleanup), `README.md`. Not credential-baked; mounts are opt-in.
