...
def test_prompt_requires_project_name_and_all_github_repositories() -> None:
    module = _load()
    prompt = module._build_prompt(
        connector_id="connector_github_test",
        project_context="CHATGPT PROJECT: Projeto Exemplo",
    )
    assert "[$github](app://connector_github_test)" in prompt
    assert "Qual é o nome do ChatGPT Project" in prompt
...
def test_combined_parser_requires_project_name_and_consistent_repo_total() -> None:
    module = _load()
...
def test_run_passes_only_when_project_context_and_github_tool_both_work(monkeypatch, tmp_path: Path) -> None:
    ...
    class FakeClient:
        def build_project_context(self, project_id, *, max_conversations, max_chars):
            ...
            return SimpleNamespace(id="g-p-test123", name="Projeto Exemplo"), "CHATGPT PROJECT: Projeto Exemplo"
    ...
    assert report["evidence"]["project_context"]["project_name_exact_match"] is True
